...
export const getEthereumProvider = (layerEndpoint: string): ethers.JsonRpcProvider => {
...
export const getBridgeContractAddress = (layerEndpoint: string): string => {
  const isMainnet = layerEndpoint.includes('mainnet.tellorlayer.com')
  const address = isMainnet ? BRIDGE_CONTRACT_ADDRESSES.mainnet : BRIDGE_CONTRACT_ADDRESSES.testnet
